...
def get_topK(query, index, model, k, device):
    """
...
def query_index(store_path, query, model, k, device):
    '''
    ...
    # 搜索
    I = get_topK(query, index, model, k, device)
    ret = []
...
def query(k, query_str):
    ...
    # index数据的路径
    index_file_path = './backend/data/index.index'
    dim = 512
    # 向量查询，找出的相似度topk的k
    # k = 5
    # 根据索引query
    ...
    related_texts = query_index(index_file_path, [query_str], model, k, device)
